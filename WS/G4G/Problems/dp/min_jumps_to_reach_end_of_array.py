...
Output: 3 (1-> 3 -> 8 ->9)
First element is 1, so can only go to 3. Second element is 3, so can make
at most 3 steps eg to 5 or 8 or 9

The idea is to find min_steps[n - 1]. Lets say some elements at indexes a, b and c
have values that makes it possible to reach n - 1 in one hop. So they are potential
predecessors to the last hop. Our goal is to choose a predecessor that minimises the
number of hops required to reach n - 1. which is min (min_steps[a], min_steps[b], min_steps[c]) + 1
"""
