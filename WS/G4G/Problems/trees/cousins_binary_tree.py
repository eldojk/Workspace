...
def get_sibling_and_level(root, key, level):
    ...
    l_sib, l_res = get_sibling_and_level(root.left, key, level + 1)
    r_sib, r_res = get_sibling_and_level(root.right, key, level + 1)

    return (l_sib, l_res) if l_res != -1 else (r_sib, r_res)
